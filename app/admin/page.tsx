...
export default async function AdminPage() {
  ...
  const { data: movies, error: moviesError } = await supabase
    .from('movies')
    .select(
      'id,title,description,poster_url,trailer_url,is_now_playing,is_coming_soon,rating,showtime_display'
    )
    ...
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-[#050505] px-4 text-center">
        ...
        <p className="mt-4 text-xs text-white/40">
          ...
          `movies`, or sign in with a manager account.
        </p>
      </div>
    )
  }
  ...
  return <AdminDashboard initialMovies={normalizedMovies} />
}
